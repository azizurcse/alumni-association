...
<?php
	...
	/**
	 * Loads the scripts required to generate the shortcode into the Gutenberg Editor
	 */
	public function gutenberg_editor()
	{
		wp_enqueue_script('cpmp-gutenberg-editor', CPMP_PLUGIN_URL.'/js/gutenberg.js', array('wp-blocks', 'wp-element'));
	...
	function replace_shortcode($atts){
		...
		if(isset($id)){
			...
			if($player != null){
                ...
                wp_enqueue_style('codepeople_media_player', plugin_dir_url(__FILE__).'css/cpmp.css');
				...
				return '<div id="ms_avp" '.$styles.'><'.$config_obj->type.' id="'.esc_attr($id.$sub_id).'" '.implode(' ', $mp_atts).'>'.implode('',$srcs).implode('', $mp_subtitles).'</'.$config_obj->type.'>'.((count($pl_items) > 0) ? '<ul id="'.esc_attr($id.$sub_id).'-list" style="display: '.((isset($config_obj->playlist) && $config_obj->playlist) ? 'block' : 'none').' !important;">'.implode(' ', $pl_items).'</ul>' : '').'<noscript>audio-and-video-player require JavaScript</noscript></div>';
